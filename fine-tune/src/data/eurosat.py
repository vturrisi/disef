...
class EuroSAT(BaseDataModule):
    ...
    def classes(self):
        if not self._classes:
            metadata_fp = Path(self.hparams.artifact_dir, "eurosat", "metadata.csv")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        metadata_fp = Path(self.hparams.artifact_dir, "eurosat", "metadata.csv")
        split_fp = Path(self.hparams.artifact_dir, "eurosat", "split_coop.csv")

        # read folder names
        metadata_df = pd.read_csv(metadata_fp)
        folders_to_idx = {str(c): i for i, c in enumerate(metadata_df["folder_name"].tolist())}
        ...
        for split in ["train", "val", "test"]:
            ...
            labels = [folders_to_idx[c] for c in folder_names]
