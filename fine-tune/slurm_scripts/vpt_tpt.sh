...
augment_idx=$(((($SLURM_ARRAY_TASK_ID) / (${#batch_sizes[@]} * ${#seeds[@]} )) % ${#augments[@]}))
batch_size_idx=$(((($SLURM_ARRAY_TASK_ID) / ${#seeds[@]}) % ${#batch_sizes[@]}))
...
echo "dataset: $dataset, augment: $augment, mixup: $mixup, cutmix: $cutmix, lr: $lr, batch_size: $batch_size, seed: $seed, visual_prompts: $vp, text_prompts: $tp"
...
python3 main.py \
    ...
    name="$dataset-base_new=$base_new-vpt_tpt-16-shot-bs=$batch_size-lr=$lr-wd=$weight_decay-ep=$epochs-sched=$sched-warmup_epochs=$warmup_epochs-mixup=$mixup-cutmix=$cutmix-label_smoothing=$label_smoothing-vp=$vp_enabled($vp)-tp=$tp_enabled($tp)-synthetic=$synthetic" \
    mixing.enabled=True \
    mixing.mixup=$mixup \
