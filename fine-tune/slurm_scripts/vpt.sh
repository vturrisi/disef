...
visual_prompts=(4 8 16 32)
seeds=(1 5 10)

dataset_idx=$(((($SLURM_ARRAY_TASK_ID) / (${#batch_sizes[@]} * ${#seeds[@]} * ${#lrs[@]} * ${#augments[@]} * ${#visual_prompts[@]})) % ${#datasets[@]}))
lr_idx=$(((($SLURM_ARRAY_TASK_ID) / (${#batch_sizes[@]} * ${#seeds[@]} * ${#augments[@]} * ${#visual_prompts[@]})) % ${#lrs[@]}))
augment_idx=$(((($SLURM_ARRAY_TASK_ID) / (${#batch_sizes[@]} * ${#seeds[@]} * ${#visual_prompts[@]})) % ${#augments[@]}))
batch_size_idx=$(((($SLURM_ARRAY_TASK_ID) / (${#seeds[@]} * ${#visual_prompts[@]})) % ${#batch_sizes[@]}))
visual_prompt_idx=$(((($SLURM_ARRAY_TASK_ID) / ${#seeds[@]}) % ${#visual_prompts[@]}))
...
vp=${visual_prompts[$visual_prompt_idx]}
seed=${seeds[$seed_idx]}

vp_enabled=$((vp > 0))

echo "dataset: $dataset, augment: $augment, mixup: $mixup, cutmix: $cutmix, lr: $lr, batch_size: $batch_size, seed: $seed, visual_prompts: $vp"
...
python3 main.py \
    ...
    name="$dataset-base_new=$base_new-vpt-16-shot-bs=$batch_size-lr=$lr-wd=$weight_decay-ep=$epochs-sched=$sched-warmup_epochs=$warmup_epochs-mixup=$mixup-cutmix=$cutmix-label_smoothing=$label_smoothing-vp=$vp_enabled($vp)-synthetic=$synthetic" \
    mixing.enabled=True \
    mixing.mixup=$mixup \
